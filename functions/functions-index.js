...
- Se a pessoa enviar uma foto do cabelo ou couro cabeludo, use-a apenas para observar textura, brilho, frizz, oleosidade, comprimento e sinais visíveis ligados ao cuidado capilar. Nunca comente sobre o rosto ou identidade da pessoa, nunca tente identificar quem é, estimar idade ou qualquer atributo pessoal — isso está fora do seu escopo. Se a foto mostrar sinal de problema de pele/couro cabeludo (feridas, vermelhidão, descamação), aplique o mesmo guard-rail de saúde acima.
...
1. Shampoo Líquido Oroterapia Sem Sal (500ml) — tecnologia Pep-Tive Gold
...
2. Máscara Oroterapia (Máscara Teia, 500g) — tecnologia Pep-Tive Gold
...
3. Óleo Reparador de Pontas Oroterapia (Oil Gold, 50ml) — tecnologia Pep-Tive Gold
...
exports.chatWithAI = onCall(
  ...
  async (request) => {
    ...
    // Validação de blocos de imagem (mensagens com foto vêm como array de content blocks).
    // Nada aqui é salvo em disco/banco — só valida e repassa pra API, a imagem não persiste.
    const ALLOWED_IMAGE_TYPES = ["image/jpeg", "image/png", "image/webp"];
    const MAX_IMAGE_BASE64_CHARS = 6 * 1024 * 1024; // ~6MB em base64 (arquivo real bem menor)
        ...
        for (const block of msg.content) {
          if (block && block.type === "image") {
            ...
            if (!ALLOWED_IMAGE_TYPES.includes(mediaType)) {
              throw new HttpsError("invalid-argument", "Tipo de imagem não suportado.");
            }
            if (!data || typeof data !== "string" || data.length > MAX_IMAGE_BASE64_CHARS) {
              throw new HttpsError("invalid-argument", "Imagem inválida ou grande demais.");
            }
          } else if (block && block.type !== "text") {
            throw new HttpsError("invalid-argument", "Tipo de conteúdo não suportado.");
          }
        }
      }
    }
