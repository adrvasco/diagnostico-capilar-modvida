...
1. Acesse https://console.firebase.google.com e crie um projeto novo (ex: `diagnostico-capilar-karseell`)
...
```bash
firebase use --add
# selecione o projeto diagnostico-capilar-karseell criado no passo 1
...
```bash
# crie um repositório novo no GitHub (ex: diagnostico-capilar-karseell)
git init
git add .
git commit -m "Primeira versão do diagnóstico capilar"
git branch -M main
git remote add origin https://github.com/SEU_USUARIO/diagnostico-capilar-karseell.git
git push -u origin main
```

Depois, no repositório do GitHub: **Settings > Pages > Branch: main / (root)** e salvar. Em alguns minutos o site fica disponível em `https://SEU_USUARIO.github.io/diagnostico-capilar-karseell`.
